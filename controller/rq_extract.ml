...
(* Marker on the autogenerated [intros] line of a generated proof: it warns the
   user the line is rewritten on every re-extraction, and lets us find that line
   again precisely. *)
let autogen_note = "DON'T EDIT THIS LINE -- AUTOGENERATED"

let contains hay sub = find_substr hay sub 0 <> None

let leading_ws l =
  let k = ref 0 in
  while !k < String.length l && (l.[!k] = ' ' || l.[!k] = '\t') do
    incr k
  done;
  String.sub l 0 !k
...
(* On re-extraction the proof file is NOT regenerated (it may hold real work), but
   ...
   the recorded binder names no longer match. Rewrite the autogenerated tactic line
   of the main [Lemma <name>_proof] to [intros_stmt], preserving its indentation and
   leaving the rest of the body untouched. We find that line by its AUTOGENERATED
   marker, falling back to the first whole-word [intros] after the lemma (for proof
   files generated before the marker existed). Returns true if it rewrote a line. *)
let update_proof_intros ~proof_path ~name ~intros_stmt =
  ...
  let li = ref 0 in
  let anchor = Printf.sprintf "Lemma %s_proof" name in
  while !li < n && not (contains arr.(!li) anchor) do
    incr li
  done;
  if !li >= n then false
  else begin
    ...
    while !target < 0 && !i < n do
      if contains arr.(!i) autogen_note then target := !i else incr i
    done;
    if !target < 0 then begin
      let i = ref !li in
      while !target < 0 && !i < n do
        match find_word arr.(!i) "intros" 0 with
        | Some _ -> target := !i
        | None -> incr i
      done
    end;
    if !target < 0 then false
      ...
      let newl = leading_ws arr.(!target) ^ intros_stmt in
      if String.equal newl arr.(!target) then false
      else begin
        arr.(!target) <- newl;
        write_file proof_path (String.concat "\n" (Array.to_list arr));
        true
      end
    end
  end
...
let generate ~(doc : Doc.t) ~point ~name (ex : Coq.State.Extract.t) =
  ...
  in
  (* The whole tactic line, marked so the user knows re-extraction rewrites it. *)
  let intros_stmt = Printf.sprintf "%s. (* %s *)" intros_text autogen_note in
  let body =
    match sec with
    | Some (_, sec_name) ->
        ...
        | Some (_, sec_name) ->
          ...
          Printf.sprintf
            ...
            \  (* Move the original tactics here to prove it for real. *)\n\
             Admitted.\n\
             End %s.\n"
            requires goal_mod sec_name preamble name goal_ref intros_stmt
            sec_name
        | None ->
          ...
          Printf.sprintf
            ...
            \  %s\n\
             Admitted.\n"
            requires goal_mod name name intros_stmt
      in
      write_file proof_path proof_src;
      false)
    else update_proof_intros ~proof_path ~name ~intros_stmt
  in
  `Assoc
